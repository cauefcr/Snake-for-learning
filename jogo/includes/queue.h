typedef struct queue {
    ...
    int first;   // indice do primeir valor preenchido
    int last;    // indice do ultimo valor preenchido
    int size;    // tamanho atual da array
} queue;
...
int queue_len(queue* que) {
    ...
    if (que->last == que->first &&
        que->last == 0) {  // se estiver vazio, tamanho 0
        return 0;
...
void queue_print(queue* que, bool dbg) {
    if (dbg) {
        ...
        wprintf(
            L" tamanho: %d, prim: %d, ult: %d, size: %d, len: %d Em ordem:\n",
...
// int main() { //testes da fila, quando era uma fila de inteiros
